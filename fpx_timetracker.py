"""
FPX TimeTracker – tkinter Popover / Großfenster.
Wird von fpx_menubar.py (Mac) oder fpx_tray.py (Windows) als Subprocess gestartet.
"""
import sys as _sys_pre
if _sys_pre.platform == 'win32':
    try:
        import ctypes as _ct_pre
        _ct_pre.windll.shcore.SetProcessDpiAwareness(1)
        del _ct_pre
    except Exception:
        pass
del _sys_pre
...
from config import (
    APP_VERSION, IS_WIN, IS_MAC,
    FONT_MAIN, FONT_MONO,
    DATA_DIR, DATA_FILE, IPC_FILE,
    write_ipc, read_ipc,
    SCALE, s, get_work_area, asset_path,
)
...
PROJEKTMANAGER = [
    {"name":"Anna Embach",         "kuerzel":"AE","urno":632 },
    {"name":"Bastian Brezinski",   "kuerzel":"BB","urno":21  },
    {"name":"Daniel Losch",        "kuerzel":"DL","urno":1411},
    {"name":"David Winkler",       "kuerzel":"DW","urno":8   },
    {"name":"Goezde Dincgez",      "kuerzel":"GD","urno":1531},
    {"name":"Katharina Schweigert","kuerzel":"KJ","urno":1199},
    {"name":"Lea Schuster",        "kuerzel":"LS","urno":954 },
    {"name":"Marcus Tischler",     "kuerzel":"MT","urno":9   },
...
def ceil_min(sec):     return math.ceil(sec / 60) if sec > 0 else 0
...
def fmt_timer(sec):
    total = int(sec); h, r = divmod(total, 3600); m, s2 = divmod(r, 60)
...
# ─────────────────────────────────────────────────────────────────────────────
class SetupWindow:
    def __init__(self, shared_root: tk.Tk):
        self.result = None
        self.win = tk.Toplevel(shared_root)
        self.win.title("FPX TimeTracker"); _apply_icon(self.win)
        self.win.update_idletasks()
        sw, sh = self.win.winfo_screenwidth(), self.win.winfo_screenheight()
        w, h = s(440), min(s(700), int(sh * 0.82))
        self.win.geometry(f"{w}x{h}+{(sw-w)//2}+{(sh-h)//2}")
        self.win.configure(bg=C["bg"]); self.win.resizable(True, True); self.win.minsize(s(380), s(480))
        self._build()
        _set_dark_titlebar(self.win)
        shared_root.wait_window(self.win)

    def _build(self):
        outer = tk.Frame(self.win, bg=C["bg"]); outer.pack(fill="both", expand=True)
        sb = ttk.Scrollbar(outer, orient="vertical"); sb.pack(side="right", fill="y")
        cv = tk.Canvas(outer, bg=C["bg"], highlightthickness=0, bd=0, yscrollcommand=sb.set)
        sb.config(command=cv.yview); cv.pack(side="left", fill="both", expand=True)
        ...
        dot.create_oval(2,2,42,42, fill=C["accent"], outline="")
        dot.create_text(22,22, text="F", font=(FONT_MAIN,19,"bold"), fill=C["text"])
        ...
        for pm in PROJEKTMANAGER:
            l = tk.Label(lf, text=pm["name"], font=(FONT_MAIN,13), bg=C["card"], fg=C["text"], anchor="w", padx=14, pady=6)
            l.pack(fill="x", pady=1); l.bind("<Button-1>", lambda e, n=pm["name"]: sel(n)); self._lbls[pm["name"]] = l
        sel(PROJEKTMANAGER[0]["name"])
        btn = tk.Label(body, text="Speichern & Starten", font=(FONT_MAIN,13,"bold"), bg=C["accent"], fg=C["bg"], padx=16, pady=10, cursor="arrow")
        btn.pack(fill="x", pady=(16,24))
    ...
    def _save(self):
        ...
        if not key:
            from tkinter import messagebox; messagebox.showwarning("Fehler", "API-Key eingeben.", parent=self.win); return
        pm = next((p for p in PROJEKTMANAGER if p["name"] == self.pm_var.get()), None)
        if not pm: return
        d = load_data()
        d["config"] = {"api_key":key,"person_urno":pm["urno"],"person_name":pm["name"],"person_kuerzel":pm["kuerzel"]}
        save_data(d); self.result = d["config"]; self.win.destroy()
...
class FPXTimeTracker:
    def __init__(self, config, shared_root: tk.Tk):
        ...
        sessions.setdefault(today_str(), {})
        self.data.setdefault("descriptions", {}); self.data.setdefault("pending_status", {})
        self.data.setdefault("booked_today", {}); self.data.setdefault("last_tracked", [])
        _today = today_str()
        ...
        self._pending = dict(self.data.get("pending_status", {}))
        self._quick_track = set(); self._force_today = set()
        ...
        self._rebuilding = False; self._qe_busy = False
        self._build_window(shared_root); self._build_ui(); self._start_clock()
        if POPOVER_MODE: self._start_ipc_loop()
        self._load_todos()

    # ── Data helpers ──────────────────────────────────────────────────────────
    def _get_parts(self, u): return self.data["sessions"].get(today_str(), {}).get(str(u), [])
    def _get_minutes(self, u): return sum(p.get("minutes", 0) for p in self._get_parts(u))
    def _get_base_secs(self, u): return self._get_minutes(u) * 60

    def _add_part(self, u, st, et, m):
    ...
    def _del_part(self, u, i):
        p = self.data["sessions"].get(today_str(), {}).get(str(u), [])
        if 0 <= i < len(p): p.pop(i); save_data(self.data)

    def _get_desc(self, u): return self.data.get("descriptions", {}).get(str(u), "")
    def _set_desc(self, u, t): self.data.setdefault("descriptions", {})[str(u)] = t; save_data(self.data)
    def _get_pending(self, u): return self._pending.get(str(u))

    def _set_pending(self, u, st):
        if st is None: self._pending.pop(str(u), None)
        else: self._pending[str(u)] = st
        self.data["pending_status"] = self._pending; save_data(self.data)

    def _toggle_pending(self, u, st):
        if self._desc_open and hasattr(self, "_desc_widget"):
            try: self._set_desc(self._desc_open, self._desc_widget.get("1.0", "end-1c").strip())
            except Exception: pass
        self._set_pending(u, None if self._get_pending(u) == st else st)
        self._render_list()

    def _toggle_quick(self, u, event=None):
        if u in self._quick_track:
            self._quick_track.discard(u)
            self.data.setdefault("quick_hours", {}).pop(str(u), None); save_data(self.data)
        else:
    ...
    # ── Window setup ─────────────────────────────────────────────────────────
    def _build_window(self, shared_root: tk.Tk):
        self.root = shared_root; self.root.title("FPX TimeTracker"); _apply_icon(self.root)
        ...
        if POPOVER_MODE:
            if IS_WIN:
                ...
                work_h = wb - wt
                self._W = s(380); self._H = min(s(740), int(work_h * 0.93))
                pad = s(8); tb_reserve = s(50)
                if edge == "bottom": self._X = wr - self._W - pad; self._Y = sh - self._H - tb_reserve; self._anim_dir = "up"
                elif edge == "top":  self._X = wr - self._W - pad; self._Y = wt + pad;                  self._anim_dir = "down"
                elif edge == "left": self._X = wl + pad;           self._Y = sh - self._H - tb_reserve; self._anim_dir = "right"
                else:                self._X = wr - self._W - pad; self._Y = sh - self._H - tb_reserve; self._anim_dir = "left"
                self._TBH = 0
                self.root.geometry(f"{self._W}x{self._H}+{self._X}+{self._Y}")
                self.root.configure(bg=C["bg"]); self.root.overrideredirect(True)
                self.root.attributes("-topmost", True); self.root.withdraw()
            else:  # Mac: Titelbar hinter Menüleiste verstecken
                self._W, self._H = 380, min(660, int(sh * 0.76))
                self._X = sw - self._W - 8; self._Y = 24; self._TBH = 28; self._anim_dir = "mac"
                self.root.geometry(f"{self._W}x{self._H + self._TBH}+{self._X}+{-self._TBH}")
                self.root.configure(bg=C["bg"]); self.root.overrideredirect(False)
                self.root.attributes("-topmost", True); self.root.withdraw()
                try:
                    from AppKit import NSApp, NSImage, NSApplicationActivationPolicyRegular
                    NSApp.setActivationPolicy_(NSApplicationActivationPolicyRegular)
                    _ico = os.path.join(os.path.dirname(__file__), "Logo", "Mac", "app.icns")
                    if os.path.exists(_ico):
                        _img = NSImage.alloc().initWithContentsOfFile_(_ico)
                        if _img: NSApp.setApplicationIconImage_(_img)
                except Exception: pass
                self.root.createcommand('::tk::mac::ReopenApplication', self._dock_toggle)
                self.root.createcommand('::tk::mac::Quit', self._dock_quit)
                self.root.protocol("WM_DELETE_WINDOW", self._animate_hide)
            write_ipc({"visible_state": "hidden"})
        else:
            w, h = s(460), min(s(800), int(sh * 0.88))
            self.root.geometry(f"{w}x{h}+{sw - w - s(40)}+{(sh - h) // 2}")
            self.root.configure(bg=C["bg"]); self.root.resizable(False, True)
            self.root.minsize(s(460), s(420))
    ...
    def _start_ipc_loop(self):
        ...
        def poll():
            try:
                d = read_ipc(); cmd = d.get("cmd", ""); ts = d.get("ts", 0)
    ...
    def _dock_quit(self):
        write_ipc({"quit_all": True, "ts": time.time()})
        self.root.after(400, self.root.destroy)

    # ── Animations ────────────────────────────────────────────────────────────
    def _slide_offset(self, t: float, dist: int) -> tuple:
    ...
    def _animate_show(self):
        if IS_MAC:
            H = self._H + self._TBH; ty = self._Y - self._TBH; dist = 25
            self.root.geometry(f"{self._W}x{H}+{self._X}+{ty - dist}")
            self.root.attributes("-alpha", 0.0); self.root.deiconify(); self.root.lift(); self.root.focus_force()
            def step(i):
                if i > 16:
                    self.root.geometry(f"{self._W}x{H}+{self._X}+{ty}")
                    self.root.attributes("-alpha", 1.0); write_ipc({"visible_state": "shown"}); return
                t = 1 - (1 - i/16)**3
                self.root.geometry(f"{self._W}x{H}+{self._X}+{int(ty - dist + dist*t)}")
    ...
    def _animate_hide(self):
        if IS_MAC:
            H = self._H + self._TBH; ty = self._Y - self._TBH; dist = 20
            def step(i):
                if i > 10:
                ...
                self.root.geometry(f"{self._W}x{H}+{self._X}+{int(ty - dist*t)}")
    ...
    def _build_ui(self):
        ...
        top = tk.Frame(self.root, bg=C["panel"], height=s(44) if POPOVER_MODE else s(54))
        top.pack(fill="x"); top.pack_propagate(False)
        dot = tk.Canvas(top, width=s(34), height=s(34), bg=C["panel"], highlightthickness=0)
        dot.pack(side="left", padx=(s(14),0), pady=s(5) if POPOVER_MODE else s(10))
        dot.create_oval(2,2,s(34)-2,s(34)-2, fill=C["accent"], outline="")
        dot.create_text(s(34)//2, s(34)//2, text="F", font=(FONT_MAIN,15,"bold"), fill=C["text"])
        ...
        rc_sz = s(26)
        rc = tk.Canvas(top, width=rc_sz, height=rc_sz, bg=C["panel"], highlightthickness=0, cursor="arrow")
        rc.pack(side="right", padx=(0,s(10)), pady=s(9))
        def drc(bg):
            rc.delete("all"); rc.create_oval(0,0,rc_sz,rc_sz, fill=bg, outline="")
            rc.create_text(rc_sz//2, rc_sz//2, text="↺", font=(FONT_MAIN,12,"bold"), fill=C["text"])
        ...
        self._clock_lbl = tk.Label(top, text="", font=(FONT_MONO,11), bg=C["panel"], fg=C["text"])
        self._clock_lbl.pack(side="right", padx=(0,s(12)))
        tk.Frame(self.root, bg=C["border"], height=1).pack(fill="x")
        self._timer_canvas = tk.Canvas(self.root, height=s(66), bg=C["card"], highlightthickness=0, bd=0)
        self._timer_canvas.pack(fill="x")
        ...
        self._canvas = tk.Canvas(lo, bg=C["bg"], highlightthickness=0, bd=0, yscrollcommand=self._sb_w.set)
        self._sb_w.config(command=self._canvas.yview); self._canvas.pack(side="left", fill="both", expand=True)
        self._list_f = tk.Frame(self._canvas, bg=C["bg"])
        self._cwin = self._canvas.create_window((0,0), window=self._list_f, anchor="nw")
        ...
        self.root.bind_all("<Button-4>", self._on_scroll); self.root.bind_all("<Button-5>", self._on_scroll)
        tk.Frame(self.root, bg=C["border"], height=1).pack(fill="x")
        export_text = ("  ✓  Tag abschliessen & nach PROAD übertragen  " if IS_MAC
                       else ("  ✓  Tag abschliessen  " if POPOVER_MODE
                             else "  ✓  Tag abschliessen & nach PROAD übertragen  "))
        if IS_MAC:
            foot = tk.Frame(self.root, bg=C["panel"], height=54); foot.pack(fill="x"); foot.pack_propagate(False)
            self._export_btn = tk.Label(foot, text=export_text, font=(FONT_MAIN,12,"bold"), bg=C["green"], fg=C["bg"], pady=9, cursor="arrow")
            self._export_btn.pack(fill="x", padx=10, pady=5)
        else:
            foot = tk.Frame(self.root, bg=C["panel"]); foot.pack(fill="x")
            self._export_btn = tk.Label(foot, text=export_text, font=(FONT_MAIN,12,"bold"), bg=C["green"], fg=C["bg"], pady=s(11), cursor="arrow")
            self._export_btn.pack(fill="x", padx=s(10), pady=s(6))
    ...
    def _build_qe(self):
        ...
        self._qe_var = tk.StringVar(); ph = "FPX-422 GRA 0.5"; self._qe_ph = ph
        qe = tk.Entry(qf, textvariable=self._qe_var, font=(FONT_MAIN,11), bg=C["card"], fg=C["text"],
                      insertbackground=C["accent"], relief="flat", bd=0, width=26)
        qe.pack(side="left", ipady=4, ipadx=6, pady=6)
        ...
        btn = tk.Label(qf, text="Erstellen", font=(FONT_MAIN,11), bg=C["accent"], fg=C["text"], padx=10, pady=4, cursor="arrow")
        btn.pack(side="left", padx=(6,0), pady=6)
        self._qe_btn = btn
        btn.bind("<Button-1>", lambda e: self._quick_entry())
        btn.bind("<Enter>", lambda e: btn.config(bg=C["accent2"]) if not self._qe_busy else None)
        btn.bind("<Leave>", lambda e: btn.config(bg=C["accent"]) if not self._qe_busy else None)
        tk.Frame(self.root, bg=C["border"], height=1).pack(fill="x")
    ...
    def _draw_timer_block(self):
        ...
        if self.timer_running and self.active_urno:
            ...
            elapsed = time.time() - self.timer_start; total_sec = self._get_base_secs(self.active_urno) + elapsed
    ...
    def _start_pulse(self):
        ...
        def pulse():
            ...
            if self._ipc_tick >= 20:
                self._ipc_tick = 0
                try:
                    elapsed = time.time() - self.timer_start; total_sec = int(self._get_base_secs(self.active_urno) + elapsed)
                    h, r = divmod(total_sec, 3600); m, s2 = divmod(r, 60)
                    txt = f"{h}:{m:02d}:{s2:02d}" if h > 0 else f"{m:02d}:{s2:02d}"
                    d = read_ipc()
                    at = self.todos_today + self.todos_overdue
                    todo = next((t for t in at if t.get("urno") == self.active_urno), None)
                    proj_no = ((todo.get("project") or {}).get("projectno", "") if todo else "")
    ...
    def _fetch_todos(self):
        try:
            ...
            r1 = requests.get(f"{PROAD_URL}/tasks", headers=self.hdr,
                              params={"person":str(self.person_urno),"from_date":f"{df}--{today}"},
                              timeout=25, verify=SSL_VERIFY); r1.raise_for_status()
            t1 = r1.json().get("todo_list", [])
            r2 = requests.get(f"{PROAD_URL}/tasks", headers=self.hdr,
                              params={"person":str(self.person_urno),"until_date":today},
    ...
    def _on_load_done(self):
        if not self.load_error:
            for u in self._booked_today:
                self.data.get("descriptions", {}).pop(str(u), None)
    ...
    def _render_list(self):
        ...
        try:
            ...
            if self.loading:
                ...
                rb = tk.Label(self._list_f, text="Erneut versuchen", font=(FONT_MAIN,12), bg=C["accent"], fg=C["bg"], padx=14, pady=7, cursor="arrow")
                rb.pack(pady=16); rb.bind("<Button-1>", lambda e: self._load_todos())
                ...
                if self.todos_overdue:
                    self._render_group_header("Abgelaufen", sum(self._get_minutes(t.get("urno",0)) for t in self.todos_overdue), C["orange"])
                    seen_d = []
                    for t in self.todos_overdue:
                        fd = (t.get("until_datetime") or t.get("from_datetime") or "")[:10]
                        if fd not in seen_d: seen_d.append(fd); self._render_date_sep(fd, fd in self._collapsed_dates)
                        if fd not in self._collapsed_dates: self._render_row(t, overdue=True)
                if self.todos_today:
                    self._render_group_header("Heute", sum(self._get_minutes(t.get("urno",0)) for t in self.todos_today), C["text_mid"])
                    for t in self.todos_today: self._render_row(t)
            ...
            self._bind_scroll_tree(self._list_f)
        finally:
            if hwnd:
                try:
    ...
    def _render_group_header(self, label, total=0, color=None):
        color = color or C["text_mid"]
        hf = tk.Frame(self._list_f, bg=C["bg"], height=36); hf.pack(fill="x"); hf.pack_propagate(False)
    ...
    def _render_date_sep(self, ds, collapsed):
        try:
        ...
        for w in [sf, al, ll]: w.bind("<Button-1>", lambda e, x=ds: self._toggle_date_collapsed(x)); w.config(cursor="arrow")

    def _toggle_date_collapsed(self, ds):
        (self._collapsed_dates.discard(ds) if ds in self._collapsed_dates else self._collapsed_dates.add(ds))
        self._render_list()

    def _render_row(self, todo, overdue=False):
        u = todo.get("urno"); ia = self.timer_running and self.active_urno == u
        bkd = str(u) in self._booked_today; parts = self._get_parts(u)
        dop = self._desc_open == u; sdesc = self._get_desc(u); pend = self._get_pending(u); isq = u in self._quick_track
        ...
        body = tk.Frame(outer, bg=rbg); body.pack(side="left", fill="both", expand=True, padx=(10,10), pady=(10,8) if IS_MAC else (7,5))
        ...
        # Zeile 0: Projektnummer + optional "heute"-Button bei überfälligen
        r0 = tk.Frame(body, bg=rbg); r0.pack(fill="x")
        tk.Label(r0, text=pno, font=(FONT_MAIN, 11 if IS_MAC else 9), bg=rbg, fg=C["accent"], anchor="w").pack(side="left")
        if overdue:
            itf = u in self._force_today
            tb = tk.Label(r0, text="heute", font=(FONT_MAIN, 10 if IS_MAC else 9),
                          bg=C["accent"] if itf else C["card2"], fg=C["text"] if itf else C["text_dim"],
                          padx=5, pady=1, cursor="arrow"); tb.pack(side="left", padx=(6,0))
        ...
        # Zeile 1: Todo-Name + Stift + Play/Pause
        ...
        if IS_MAC:
            ...
            for w in [pf, pl]:
                w.bind("<Button-1>", lambda e, x=u: self._toggle_timer(x))
            ...
            _draw_icon()
            pf.bind("<Button-1>", lambda e, x=u: self._toggle_timer(x))
        ...
        tk.Label(nf, text=todo.get("shortinfo","?"),
                 font=(FONT_MAIN, 12 if IS_MAC else 11, "bold") if not bkd else (FONT_MAIN, 12 if IS_MAC else 11),
                 bg=rbg, fg=C["text"] if not bkd else C["text_mid"],
                 anchor="w", wraplength=220, justify="left").pack(side="left")
        dl = tk.Label(nf, text="✎", font=(FONT_MAIN,11), bg=rbg, fg=C["yellow"], cursor="arrow", padx=2)
        dl.pack(side="left", anchor="n", pady=1); dl.bind("<Button-1>", lambda e, x=u: self._toggle_desc(x))
        ...
        db.bind("<Button-1>", lambda e, x=u: self._toggle_pending(x,"erledigt"))
        ...
        wb.bind("<Button-1>", lambda e, x=u: self._toggle_pending(x,"wartet"))
        ...
        qb = tk.Label(r2, text="★ Tracken" if qa else "Tracken", font=(FONT_MAIN,10), bg=qb_bg, fg=C["text"], padx=8, pady=2, cursor="arrow"); qb.pack(side="left")
        qb.bind("<Button-1>", lambda e, x=u: self._toggle_quick(x,e))
        qb.bind("<Enter>", lambda e, b=qb, h=qb_hbg: b.config(bg=h)); qb.bind("<Leave>", lambda e, b=qb, o=qb_bg: b.config(bg=o))
        ...
        if isq:
            ...
            qh_e = tk.Entry(r2, textvariable=qh_var, font=(FONT_MONO,11), bg=C["card2"],
                            fg=C["text"], insertbackground=C["text"], relief="flat", bd=0, justify="center", width=4)
            ...
            def _save_qh(e=None, x=u, v=qh_var):
                try:
                    hv = float(v.get().replace(",","."))
                    if hv > 0: self.data.setdefault("quick_hours",{})[str(x)] = hv; save_data(self.data)
                except Exception: pass
            qh_e.bind("<Return>", _save_qh); qh_e.bind("<FocusOut>", _save_qh)
        if parts:
            pt = tk.Label(r2, text=f"{'▼' if u in self._expanded else '▶'} {len(parts)}",
                          font=(FONT_MAIN,10), bg=rbg, fg=C["green"], cursor="arrow", padx=4)
            pt.pack(side="right", padx=(0,4)); pt.bind("<Button-1>", lambda e, x=u: self._toggle_expanded(x))
        if scn or hl is not None:
            if hl is not None:
                hl_color = C["red"] if hl == 0 else C["text"]; det_txt = f"{hl:g}h"
            else:
        ...
        # Zeile 3: Beschreibungsvorschau / übertragen-Badge
        r3 = tk.Frame(body, bg=rbg); r3.pack(fill="x", pady=(4,0))
        if sdesc and not dop:
            tk.Label(r3, text=f"✎  {sdesc[:62]}{'...' if len(sdesc)>62 else ''}",
                     font=(FONT_MAIN,10,"italic"), bg=rbg, fg=C["yellow"], anchor="w", wraplength=360).pack(side="left")
        if bkd:
        ...
        if dop:
            ...
            dt = tk.Text(df, font=(FONT_MAIN,11), bg=C["card"], fg=C["text"], insertbackground=C["yellow"],
                         relief="flat", bd=0, height=3, wrap="word")
            dt.pack(padx=1, pady=1, ipadx=6, ipady=4, fill="x")
            if sdesc: dt.insert("1.0", sdesc)
            self._desc_widget = dt
            sb2 = tk.Label(dfo, text="Speichern", font=(FONT_MAIN,10,"bold"), bg=C["accent2"], fg=C["text"], padx=10, pady=3, cursor="arrow")
            sb2.pack(side="right", pady=(4,0))
            def _sdesc(e=None, x=u, d=dt):
                self._set_desc(x, d.get("1.0","end-1c").strip())
        ...
        if u in self._expanded and parts:
            pf2 = tk.Frame(body, bg=rbg); pf2.pack(fill="x", pady=(5,0))
            tk.Frame(pf2, bg=C["border"], height=1).pack(fill="x", pady=(0,3))
        ...
        tk.Frame(self._list_f, bg=C["border"], height=1).pack(fill="x")
        all_ws = self._all_children(outer); snap = {w: w.cget("bg") for w in all_ws if hasattr(w,"cget")}
    ...
    def _all_children(self, w):
        r = [w]
        for child in w.winfo_children(): r.extend(self._all_children(child))
        return r

    def _toggle_expanded(self, u): (self._expanded.discard(u) if u in self._expanded else self._expanded.add(u)); self._render_list()
    def _toggle_desc(self, u): self._desc_open = None if self._desc_open == u else u; self._render_list()

    # ── Timer control ─────────────────────────────────────────────────────────
    def _toggle_timer(self, u):
    ...
    def _start_timer(self, u):
        ...
        if POPOVER_MODE: self.root.after(150, self._animate_hide)

    def _stop_timer(self, rerender=True, add_part=True):
        if not self.timer_running: return
        elapsed = time.time() - self.timer_start; u = self.active_urno
        if add_part and elapsed > 0: self._add_part(u, self.timer_start, time.time(), ceil_min(elapsed))
    ...
    def _delpart(self, u, i):
        if not (0 <= i < len(self._get_parts(u))): return
        ...
        self._del_part(u, i); self._render_list(); self._draw_timer_block()
    ...
    def _quick_entry(self):
        if self._qe_busy: return
        raw = self._qe_var.get().strip()
        fmt = "Format: FPX-422 GRA 0.5"
        if not raw or raw == self._qe_ph:
            self._dialog("Fehler", f"Bitte Eingabe machen.\n{fmt}", "error"); return
        pts = re.split(r'[\s,;]+', raw)
        if len(pts) < 2:
            self._dialog("Fehler", f"Leistungsart fehlt.\n{fmt}", "error"); return
        if len(pts) < 3:
            self._dialog("Fehler", f"Stunden fehlen.\n{fmt}", "error"); return
        proj_no = pts[0].upper(); sc_key = pts[1].upper()
        try:
            hours = float(pts[2].replace(",","."))
            if hours <= 0: raise ValueError
        except Exception:
            self._dialog("Fehler", f"Ungültige Stunden.\n{fmt}", "error"); return

        self._qe_busy = True
        self._qe_btn.config(bg=C["gray_btn"])

        def _done():
            self._qe_busy = False
            self._qe_btn.config(bg=C["accent"])

        def do():
            try:
                r = requests.get(f"{PROAD_URL}/projects", headers=self.hdr,
                                 params={"projectno": proj_no},
                                 timeout=20, verify=SSL_VERIFY); r.raise_for_status()
                proj = next((p for p in r.json().get("project_list",[]) if p.get("projectno","").upper()==proj_no), None)
                if not proj:
                    self.root.after(0, lambda: (_done(), self._dialog("Fehler", f"Projekt '{proj_no}' nicht gefunden.", "error"))); return
                rs = requests.get(f"{PROAD_URL}/service_codes", headers=self.hdr, timeout=15, verify=SSL_VERIFY)
                scl = rs.json().get("service_code_list",[]) if rs.ok else []
                sc = next((_sc for _sc in scl if _sc.get("shortname","").upper()==sc_key or _sc.get("name","").upper()==sc_key), None)
                if not sc:
                    self.root.after(0, lambda: (_done(), self._dialog("Fehler", f"'{sc_key}' nicht gefunden.", "error"))); return
                d = today_str(); pname = proj.get("project_name") or proj_no
                payload = {"shortinfo":f"PM {pname}","urno_project":proj["urno"],"urno_responsible":self.person_urno,
                           "urno_manager":self.person_urno,"status":"100","from_datetime":f"{d}T00:00:00",
                           "until_datetime":f"{d}T00:00:00","hours_planned":hours,"hours_left":hours,"urno_service_code":sc["urno"]}
                rt = requests.post(f"{PROAD_URL}/tasks", headers=self.hdr, json=payload, timeout=20, verify=SSL_VERIFY)
                if not rt.ok:
                    self.root.after(0, lambda: (_done(), self._dialog("Fehler", rt.text[:120], "error"))); return
                self.root.after(0, lambda: (_done(), self._qe_var.set(""), self._dialog("OK", f"Todo 'PM {pname}' ({hours}h) angelegt.", "ok"), self._load_todos()))
            except Exception as ex:
                self.root.after(0, lambda: (_done(), self._dialog("Fehler", str(ex)[:120], "error")))
        threading.Thread(target=do, daemon=True).start()
    ...
    def _close_day(self):
        ...
        for t in all_todos:
            u = t.get("urno")
            if u in self._quick_track and self._get_minutes(u) == 0:
                h = self.data.get("quick_hours",{}).get(str(u)) or t.get("hours_left") or 0
                if h > 0: qt_pending[u] = round(h*60)
        to_exp = []
        for t in all_todos:
            u = t.get("urno"); pend = self._get_pending(u)
            mins = self._get_minutes(u) + qt_pending.get(u,0)
            if (mins > 0 or pend) and str(u) not in self._booked_today:
                to_exp.append((t, mins))
        if not to_exp: self._dialog("Fertig", "Nichts zu exportieren.", "info"); return
        enr = []
        for to, m in to_exp:
            try:
        ...
        err, val = [], []
        for t, m in enr:
            if m == 0: val.append((t,m)); continue
        ...
        for t, m in enr:
            tot += m
            sc = t.get("service_code") or {}
            sc_u = (sc.get("urno") if isinstance(sc,dict) else sc) or t.get("urno_service_code")
            scn = get_sc_name(sc) or self.sc_map.get(sc_u,"?")
            pno = (t.get("project") or {}).get("projectno","?"); p = self._get_pending(t.get("urno"))
            time_str = fmt_hhmm(m) if m > 0 else "—"; status_str = f" [{p.upper()}]" if p else ""
            lines.append(f"  {pno}  {time_str}{status_str}\n    {t.get('shortinfo','?')[:34]}")
        if not self._askdialog("Tag abschliessen", "Zeiten übertragen:\n\n"+"\n".join(lines)+f"\n\nGesamt: {fmt_hhmm(tot)}\n\nJetzt?"): return
        for t, m in enr:
            u = t.get("urno")
            if u in qt_pending and self._get_minutes(u) == 0:
                now = time.time(); mins = qt_pending[u]
                self._add_part(u, now - mins*60, now, mins)
        ok, er = [], []
        for t, m in enr:
            u = t.get("urno"); sc = t.get("service_code") or {}
            sc_u = (sc.get("urno") if isinstance(sc,dict) else sc) or t.get("urno_service_code")
            proj = t.get("project") or {}; pno = proj.get("projectno","?"); desc = self._get_desc(u) or None
            ud = (t.get("until_datetime") or "")[:10]
            if u in self._force_today:  bd = today_str()
            elif ud and ud >= today_str(): bd = today_str()
            elif ud and ud < today_str():  bd = ud
            else: bd = today_str()
            if m > 0:
                pl = {"urno_person":self.person_urno,"urno_project":proj.get("urno"),"urno_task":u,
                      "urno_service_code":sc_u,"from_date":bd,"input":min_to_h(m),"chargeable":1}
        ...
        def push():
            for t, m in enr:
                u = t.get("urno"); p = self._get_pending(u); q = u in self._quick_track
                h_left = t.get("hours_left") or 0; h_book = min_to_h(m)
                ...
                self._quick_track.discard(u); self._set_pending(u, None)
                self._force_today.discard(u); self.data.setdefault("quick_hours",{}).pop(str(u), None)
        threading.Thread(target=push, daemon=True).start()
        self._render_list(); self._draw_timer_block()
        if er: self._dialog("Teilerfolg", f"OK: {', '.join(ok)}\nFehler:\n"+"\n".join(er), "warn")
        else:  _play_success_sound(); self._dialog("Übertragen!", f"Alle {len(ok)} Einträge übertragen.", "ok")
    ...
    def _dlg_pos(self, d):
        ...
        d.geometry(f"+{rx+(rw-dw)//2}+{ry+(rh-dh)//2}")

    def _dialog(self, title, msg, kind="info"):
        if IS_MAC:
            from tkinter import messagebox
            if kind == "error":   messagebox.showerror(title, msg, parent=self.root)
            elif kind == "warn":  messagebox.showwarning(title, msg, parent=self.root)
            else:                 messagebox.showinfo(title, msg, parent=self.root)
            return
...
def main(shared_root: tk.Tk):
    ...
    tracker = FPXTimeTracker(config, shared_root)
    tracker.run()
    if not POPOVER_MODE:
